// This beta drastically increases the speed of removing ks after testing by using indexOf instead of split, and makes initial testing output nothing so it speeds up.

var fs = require('fs'); //for reading and writing files
settings = JSON.parse(fs.readFileSync("DualRieselClient.json"))
...
function askForAlgebraicSets(isStarted) {
  if (!isStarted) {
    prompt.get(['Are there any covering sets? (y or n)'], function(err, result) {
      ...
      if (isSets == "y") {
        console.log("See this document on how enter covering sets: ")
...
function initialTesting() {
  ...
  if (minN > maxSize && minN != Infinity) {
    ...
    maxThreads = 1 //defaults to 40
    if (candidates.length <= 128 * maxThreads) {
      ...
      var numPerThread = Math.floor(candidates.length / maxThreads) + 1;
    ...
    for (var i = 0; i < candidateString.length; i++) {
      pfgw[i] = spawn(settings.pathToPFGW + ' -C"quiet" -f -T' + settings.numThreads + ' ' + SR + base + "-" + minK + "-" + maxK + "-" + N + "_" + i + ".txt", [], {
        shell: true ,detached: true
      }); //use factoring because no sieving
        ...
        if (numDone == candidateString.length) {
          ...
          if (N < maxSize) {
            ...
            for (var i = candidates.length; i >= 0; i--) {
              var k = candidates[i];
              if (primes.indexOf(base + "^" + (N - 1) + op + k + "\r\n") > -1) { //This was a BIG BUG that removed small ks for numbers that had less digits than maxK. e.g. k=1234 would get removed along with k=12345...
  ...
  if (maxK <= 2147483647) {
    ...
    async.parallel({
        one: function(callback) {
          callback(null, sieveEach(splitCandidates[0], minS, maxS, 0, "", function(err, result) {
            sieves[0] = result;
            numDone++;
            if (numDone == 3) {
        ...
        two: function(callback) {
          callback(null, sieveEach(splitCandidates[1], minS, maxS, 0, "", function(err, result) {
            sieves[1] = result;
            numDone++;
            if (numDone == 3) {
              finalizeSieve(sieves, minS, maxS)
        ...
        three: function(callback) {
          callback(null, sieveEach(splitCandidates[2], minS, maxS, 0, "", function(err, result) {
            sieves[2] = result;
            numDone++;
            if (numDone == 3) {
              finalizeSieve(sieves, minS, maxS)
...
function sieveEach(cand, minS, maxS, ind, candLeft, callback) {
  if (cand) {
    ...
    var stdbench = "";
    var pfgwBench = spawn(settings.pathToPFGW + ' -Bspec,sexp=' + base + '^^n' + operator + thisK + ",minn=" + minS + ",maxn=" + maxS, [], {
      shell: true
    });
    ...
    pfgwBench.on('exit', (code) => {
      ...
      if (exists(filename) && thisK != undefined) {
        console.log(avg)
        var newpgen = spawn(settings.pathToCNewPGen + ' -wp=SieveFiles/' + SR + base + "_" + thisK + "_" + minS + "_" + maxS + ".npg -t=" + type + " -v -base=" + base + " -k=" + thisK + " -nmin=" + minS + " -nmax=" + maxS + " -osp=" + avg, [], {
          shell: true
        });
...
function test(can, lim) {
  ...
  var pfgw = spawn(settings.pathToPFGW + ' -T' + settings.numThreads + ' -q"' + base + '^' + can[0][1] + sign + can[0][0] + '"', [], {
    shell: true
  }); //use factoring because no sieving
